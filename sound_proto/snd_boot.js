...
(function () {
  ...
  async function bootInlineSound() {
    if (!window.SoundBus && !window.SoundBusProto) await loadScript(loc("sound_bus.js"));
    var api = window.SoundBus || window.SoundBusProto || (Module && (Module.SoundBus || Module.SoundBusProto));
    if (!api || !api.create) {
      console.error("[snd-boot] SoundBus missing — copy sound_bus.js next to App.html");
      return null;
    }
